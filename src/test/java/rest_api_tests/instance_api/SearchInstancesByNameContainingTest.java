...
@Profile("Testing")
public class SearchInstancesByNameContainingTest {
	...
	@Test
	public void testAdminSearchByNameContaining() {
		UserBoundary requestingUser = this.testingFactory.createNewUser(UserRole.ADMIN);

		assertThrows(HttpClientErrorException.Forbidden.class, () -> {
			this.client.getForObject(this.url + requestingUser + "/search/byNameContaining/" + this.name,
					InstanceBoundary[].class);
		});
	}

	@Test
	public void testManagerSearchByNameContaining() {
		UserBoundary requestingUser = this.testingFactory.createNewUser(UserRole.MANAGER);

		assertThat(this.client.getForObject(this.url + requestingUser + "/search/byNameContaining/" + this.name,
				InstanceBoundary[].class)).hasSize(2);
	}
	
	@Test
	public void testManagerSearchByHalfNameContaining() {
		UserBoundary requestingUser = this.testingFactory.createNewUser(UserRole.MANAGER);

		assertThat(this.client.getForObject(this.url + requestingUser + "/search/byNameContaining/" + this.half_name,
				InstanceBoundary[].class)).hasSize(2);
	}

	@Test
	public void testPlayerSearchByNameContaining() {
	...
	@Test
	public void testNonExistingUserSearchByNameContaining() {
